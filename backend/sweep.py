...
def main():
    ...
    sweep_config = {
        ...
        'parameters': {
            # --- Model Architecture (aligned with improved config) ---
            'LEARNING_RATE': {'distribution': 'log_uniform_values', 'min': 5e-5, 'max': 5e-4},  # Much lower range
            'WEIGHT_DECAY': {'distribution': 'log_uniform_values', 'min': 1e-5, 'max': 1e-3},   # Lower range
            'HIDDEN_DIM': {'values': [128, 256, 384, 512]},                                     # Include 256 baseline
            'DROPOUT_RATE': {'distribution': 'uniform', 'min': 0.15, 'max': 0.35},             # Narrower around 0.2
            'DOMAIN_EMB_DIM': {'values': [24, 32, 48]},                                         # Lower values, include 32
            'USER_EMB_DIM': {'values': [32, 48, 64]},                                           # Lower values, include 48

            # --- Training Constants (aligned with improved config) ---
            'BATCH_SIZE': {'values': [256, 512, 768]},                                          # Include 512 baseline
            'PATIENCE': {'values': [30, 50, 75]},                                               # Higher patience values
            'FACTOR': {'distribution': 'uniform', 'min': 0.5, 'max': 0.8},                     # Include 0.7 baseline
            ...
            'NUM_EPOCHS': {'value': 150},        # Higher for lower learning rates
            'VAL_SIZE': {'value': 0.15},         # Aligned with improved config
            'TEST_SIZE': {'value': 0.15},        # Aligned with improved config
        },
        'early_terminate': {
            'type': 'hyperband',
            'min_iter': 5,
